...
Yours: """).split(" ")
    ...
    for lang in langs:
        if lang not in ["fr", "en", "de", "es"]:
            print("Error in language input! Closing...")
            exit()
        else:
            langs_dict[lang] = int(input(f"Weight for {lang} (int): "))
...
def app_evaluate(langs_dict, top_show, weight_name, thumb_row):
    ...
    for row in df_bigrams.itertuples():
        for c in punctuation:
            if str(c) in row.Index:
                df_bigrams.at[row.Index,
                              'en'] = df_bigrams.at[row.Index, 'en_perso']
                df_bigrams.at[row.Index,
                              'fr'] = df_bigrams.at[row.Index, 'fr_perso']
    ...
    df_bigram_weight = bigram_weight(
        df_layouts, df_keys, df_bigrams, df_penalties)
    ...
    for w in langs_dict.values():
        w_total += w

    for name, weight in langs_dict.items():
        if name == "fr":
            w_fr = weight / w_total
        elif name == "en":
            w_en = weight / w_total
    ...
    df_results['Personal average'] = (df_results.en * w_en +
                                      df_results.fr * w_fr +
                                      df_results.de * w_de +
                                      df_results.es * w_es)

    # normalize the results based of Qwerty Personnal average
    df_results = df_results.applymap(lambda x: round(
        x/df_results.at['Qwerty', 'Personal average'] * 100, 2))

    # sort the results
    df_results = df_results.sort_values(
        by=['Personal average'], ascending=True)
...
def app_generate(langs_dict, weight_name, thumb_row):
    ...
    while(current_score < best_score and times < limit):
        # un comment to limit
        # times += 1
        ...
        best_layout_as_str = layout_list_to_str(
            best_layout_name, best_layout_layout_list)
...
def layout_list_to_str(name, layout_list):
    """
    Utility function to format layout as list into a string to feed to the evaluation.
    """
    layout_str = f">>{name}\n"
    index = 1
...
def load_config(weight_name, thumb_row):
    ...
    for t in parts:
        ...
        parts_location.append(
            (part_location, filetext.find('\n', part_location) + 1))
    ...
    for title, block in output:
        if "w_" in title:
            if title == weight_name:
                output_w.append(("weights", block))
        else:
            output_w.append((title, block))
...
def parse_config(blocks, eval_or_gen, thumb_row):
    ...
    df_bigrams = pd.read_csv(os.path.join(os.path.dirname(
        os.path.realpath('__file__')), 'stats.csv'), header=0, sep=',', index_col=0)
    ...
    for i in range(len(blocks)):
        if blocks[i][0] == 'penalties':
            break
    df_penalties = pd.read_csv(
        StringIO(blocks[i][1]), sep=',', header=0, index_col=0, skipinitialspace=True)
...
def check_missing_letters(df_layouts, df_bigrams):
    ...
    for layout in df_missing_letters.itertuples():
        ...
        if missing_letters != '':
            df_missing_letters.at[layout.Index,
...
def make_swap_layouts(base_name, base_layout, thumb_row):
    ...
    for symbol_from in symbols_to_swap:
        for symbol_to in symbols_to_swap:
            ...
            tried = (symbol_from, symbol_to) in combination_tried or (
                symbol_to, symbol_from) in combination_tried
...
def bigram_weight(df_layouts, df_keys, df_bigrams, df_penalties):
    ...
    df_bigram_weight = pd.DataFrame(
        index=df_bigrams.index.values, columns=df_layouts.keys())
    ...
    for column in df_bigram_weight:
        for row in df_bigram_weight.itertuples():
            df_bigram_weight.at[row.Index, column] = weight(
                row.Index, column, df_layouts, df_keys, df_penalties)
...
def layout_results(df_bigrams, df_bigram_weight):
    ...
    df_results = pd.DataFrame(
        index=df_bigram_weight.keys(), columns=df_bigrams.keys())
    ...
    for column in df_results:  # language
        for row in df_results.itertuples():  # layout
            ...
            df_results.at[row.Index, column] = (
                df_bigrams[column]/100 * df_bigram_weight[row.Index]).sum()
